...
description: Create dbt unit-test YAML that mocks upstream inputs and validates expected outputs. Use when adding unit tests for a dbt model, practicing test-driven development in dbt, or reviewing warehouse-specific unit-test behavior.
---
...
- Prefer parsing and compilation before commands that create warehouse relations.
- Confirm target and selector before tests; use a disposable development target unless authorized otherwise.
- Never expose profile credentials or use production data when synthetic fixtures suffice.
