...
description: Create or modify dbt Semantic Layer components including semantic models, metrics, dimensions, entities, measures, and time spines. Use for MetricFlow configuration, metric types, semantic contracts, and validation across supported dbt specification versions.
---
...
- Determine the installed dbt and specification version from project evidence; do not guess.
- Validate YAML and compile or parse before querying a warehouse.
- Confirm grain, joins, time zone, denominator, and consumer compatibility before shared metric changes.
- Require explicit authorization before commands that write relations or change a deployed semantic contract.
