...
description: Build and modify dbt models, SQL transformations, tests, and validation workflows using ref(), source(), compile, parse, list, and limited previews. Use for dbt modeling, debugging, source exploration, tests, and impact analysis; require explicit authorization before warehouse-mutating runs.
---

# Using dbt for Analytics Engineering

## Codex safety adaptation

- Inspect the target, profile, and selectors first. Start with `parse`, `compile`, `list`, or a narrow preview.
- Treat `run`, `build`, seeds, snapshots, full refreshes, and production-target commands as warehouse writes; obtain explicit authorization.
- `--empty` can replace an existing relation with no rows. Use it only with explicit approval and a confirmed disposable target.
- Never print credentials or copy profile secrets into reports.
