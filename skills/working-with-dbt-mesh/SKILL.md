...
description: Assess and roll out potentially breaking dbt model changes across downstream models, exposures, dashboards, and BI consumers. Use for model versions, contracts, access, groups, migration windows, cross-project refs, dependencies.yml, and dbt Mesh decomposition.
---
...
- Inspect manifests, dependencies, contracts, and consumers before editing a shared interface.
- Prefer compatible versions and an explicit migration window over in-place breaking changes.
- Require authorization for access changes, deprecation, project splitting, and production dbt commands.
- Validate with parse, compile, and narrow selectors before warehouse-mutating execution.
